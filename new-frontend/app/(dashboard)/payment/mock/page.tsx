...
// app/(dashboard)/payment/mock/page.tsx
// এই page টা mock bKash payment page — real bKash এর মতো দেখায়
...
export default function MockBkashPage() {
  ...
  const handlePinSubmit = async () => {
    ...
    try {
      // Backend callback hit করো
      const res = await fetch(
        `${BASE_URL}/payment/callback?paymentID=${paymentID}&status=success`,
        { redirect: "manual" }
      )

      // Backend থেকে redirect URL নাও
      const data = await res.json().catch(() => null)

      // Success page এ redirect
      router.push(`/payment/success?amount=${amount}&trxID=TRX${Date.now()}`)
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ fontSize: "24px", fontWeight: "bold", color: "#E2136E" }}>
            ৳{amount}
          </div>
        </div>
        ...
        <div style={{ padding: "20px" }}>
            ...
            <div>
              <div style={{ marginBottom: "16px" }}>
                ...
                <div style={{ fontSize: "11px", color: "#999", marginTop: "4px" }}>
                  Test: যেকোনো ১১ সংখ্যার নম্বর
                </div>
              </div>
              {error && <div style={errorStyle}>{error}</div>}
              <button onClick={handlePhoneSubmit} style={btnStyle}>
                পরবর্তী
              </button>
            </div>
          )}
          ...
          {step === "otp" && (
            <div>
              ...
              {error && <div style={errorStyle}>{error}</div>}
              <button onClick={handleOtpSubmit} style={btnStyle}>
                যাচাই করুন
              </button>
            </div>
          )}
