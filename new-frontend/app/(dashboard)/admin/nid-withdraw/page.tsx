...
// import {
//   Eye, CheckCircle, XCircle, FileUp, Clock, User, Search,
//   Download, Copy, Check, ExternalLink, Trash2, Volume2, VolumeX,
// } from "lucide-react"
// import Swal from "sweetalert2"
// import {
//   getAllNidWithdrawRequests, nidWithdrawAccept, nidWithdrawCancel,
//   nidWithdrawMarkSeen, nidWithdrawSendPdf, nidWithdrawAdminDelete,
//   nidWithdrawAdminBulkDeleteCompleted,
// } from "@/lib/api.nidWithdraw"
...
//         <div className="flex items-center justify-between border-b border-border px-6 py-4">
//           <h2 className="text-base font-bold text-foreground">আবেদনের বিবরণ</h2>
//           <button onClick={onClose} className="flex h-7 w-7 items-center justify-center rounded-full text-muted-foreground transition-colors hover:bg-muted hover:text-foreground">✕</button>
//         </div>
//         <div className="space-y-4 px-6 py-5">
//           <div>
//             <p className="mb-3 text-[11px] font-bold uppercase tracking-wider text-muted-foreground">Applicant Details</p>
...
//         </div>
//         <div className="border-t border-border px-6 py-4 text-right">
//           <button onClick={onClose} className="rounded-lg border border-border px-5 py-2 text-sm text-muted-foreground hover:bg-muted">বন্ধ করুন</button>
//         </div>
//       </div>
//     </div>
//   )
...
// // ── Sound helper (same engine as NotificationProvider) ────────────
// function playBeep() {
//   try {
...
//   // Track seen notification IDs so we only react to new ones
...
//   // ── Socket-driven auto-refresh via shared NotificationContext ──
//   useEffect(() => {
...
//     if (!cancelModal) return
//     if (!cancelModal.note.trim() || cancelModal.note.trim().length < 3) {
//       Swal.fire({ icon: "warning", title: "বাতিলের কারণ লিখুন (কমপক্ষে ৩ অক্ষর)" }); return
//     }
//     setActionId(cancelModal.id)
//     const { id, note } = cancelModal
//     setCancelModal(null)
//     try {
//       await withToken(async (token) => {
//         const res = await nidWithdrawCancel(token, id, note)
...
//   const handleSendPdf = async () => {
//     if (!pdfModal) return
//     if (!pdfModal.url.trim()) { Swal.fire({ icon: "warning", title: "PDF URL দিন" }); return }
//     setActionId(pdfModal.id)
//     const { id, url } = pdfModal
//     setPdfModal(null)
//     try {
//       await withToken(async (token) => {
//         const res = await nidWithdrawSendPdf(token, id, url)
...
//   const handleAdminDelete = async (id: string, status: TNidWithdrawStatus) => {
//     if (!COMPLETED_STATUSES.includes(status)) {
//       Swal.fire({ icon: "warning", title: "মুছা যাবে না", text: "শুধুমাত্র সম্পন্ন আবেদন মুছা যাবে।" }); return
//     }
//     const confirm = await Swal.fire({
//       title: "Admin তালিকা থেকে সরাবেন?",
//       text: "User history অক্ষুণ্ণ থাকবে।", icon: "warning",
//       showCancelButton: true, confirmButtonText: "হ্যাঁ, সরিয়ে দিন",
//       cancelButtonText: "না", confirmButtonColor: "#6b7280",
//     })
//     if (!confirm.isConfirmed) return
//     setActionId(id)
//     try {
...
//   const handleBulkDeleteCompleted = async () => {
//     if (completedCount === 0) { Swal.fire({ icon: "info", title: "কোনো সম্পন্ন আবেদন নেই" }); return }
//     const confirm = await Swal.fire({
//       title: `${completedCount}টি সম্পন্ন আবেদন সরাবেন?`,
//       text: "User history অক্ষুণ্ণ থাকবে।", icon: "warning",
//       showCancelButton: true, confirmButtonText: "হ্যাঁ, সব সরিয়ে দিন",
//       cancelButtonText: "না", confirmButtonColor: "#6b7280",
//     })
//     if (!confirm.isConfirmed) return
//     setBulkLoading(true)
...
//             className="flex items-center gap-1.5 rounded-full border border-border bg-muted/60 px-3 py-1 text-xs font-semibold text-muted-foreground transition-colors hover:bg-muted hover:text-foreground"
//           >
...
//           </span>
//           {completedCount > 0 && (
//             <button disabled={bulkLoading} onClick={handleBulkDeleteCompleted}
//               className="flex items-center gap-1.5 rounded-full border border-gray-300 bg-gray-100 px-3 py-1 text-xs font-semibold text-gray-600 transition-colors hover:bg-gray-200 disabled:opacity-60 dark:border-gray-600 dark:bg-gray-700/40 dark:text-gray-300 dark:hover:bg-gray-700">
//               <Trash2 size={11} />
//               {bulkLoading ? "সরানো হচ্ছে..." : `Delete All Completed (${completedCount})`}
//             </button>
...
//         <div className="relative flex-1">
//           <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground" size={16} />
//           <input type="text" placeholder="নাম, NID নম্বর বা Email..."
//             className="w-full rounded-xl border border-border bg-muted/40 py-2 pl-10 pr-4 text-sm text-foreground outline-none placeholder:text-muted-foreground focus:ring-2 focus:ring-primary/20"
...
//                   <th key={h} className={`px-4 py-4 text-[11px] font-bold uppercase tracking-wider text-muted-foreground ${h === "Actions" ? "text-right" : ""}`}>{h}</th>
//                 ))}
//               </tr>
//             </thead>
//             <tbody className="divide-y divide-border">
...
//                   const isCompleted = COMPLETED_STATUSES.includes(r.status)
//                   return (
//                     <tr key={r._id} className={`group transition-colors hover:bg-muted/20 ${isCompleted ? "opacity-70" : ""}`}>
//                       <td className="px-4 py-4 text-sm text-muted-foreground">{idx + 1}</td>
//                       <td className="px-4 py-4">
//                         <div className="flex items-center gap-3">
//                           <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-primary/10 text-sm font-bold text-primary">
...
//                           </div>
//                           <div className="min-w-0">
//                             <div className="truncate text-sm font-bold text-foreground">{r.userId?.name || "Anonymous"}</div>
//                             <div className="truncate text-[11px] text-muted-foreground">{r.userId?.email}</div>
//                             <div className="text-[10px] text-muted-foreground">৳{r.userId?.wallet?.balance ?? "—"}</div>
//                           </div>
//                         </div>
//                       </td>
//                       <td className="px-4 py-4">
...
//                             </p>
//                             <button onClick={() => setDetailsRequest(r)}
//                               className="ml-auto inline-flex items-center gap-1 rounded-md border border-border bg-muted/60 px-2 py-0.5 text-[10px] font-semibold text-muted-foreground transition-all hover:border-primary/40 hover:bg-primary/5 hover:text-foreground">
//                               <Eye size={9} /> Details
//                             </button>
//                           </div>
...
//                         <div className="flex items-center justify-end gap-1.5 flex-wrap">
//                           {r.status === "pending" && (
//                             <button disabled={actionId === r._id} onClick={() => handleMarkSeen(r._id)}
//                               className="flex items-center gap-1 rounded-lg bg-blue-500 px-2.5 py-1.5 text-[11px] font-bold text-white shadow-sm transition-all hover:bg-blue-600 disabled:opacity-60">
//                               <Eye size={13} /> দেখেছেন
//                             </button>
//                           )}
//                           {(r.status === "pending" || r.status === "admin_seen") && (
//                             <button disabled={actionId === r._id} onClick={() => handleAccept(r._id)}
//                               className="flex items-center gap-1 rounded-lg bg-emerald-500 px-2.5 py-1.5 text-[11px] font-bold text-white shadow-sm transition-all hover:bg-emerald-600 disabled:opacity-60">
//                               <CheckCircle size={13} /> Accept
//                             </button>
//                           )}
//                           {["pending", "admin_seen", "accepted"].includes(r.status) && (
//                             <button disabled={actionId === r._id} onClick={() => setCancelModal({ id: r._id, note: "" })}
//                               className="flex items-center gap-1 rounded-lg bg-rose-500 px-2.5 py-1.5 text-[11px] font-bold text-white shadow-sm transition-all hover:bg-rose-600 disabled:opacity-60">
//                               <XCircle size={13} /> বাতিল
//                             </button>
//                           )}
//                           {["pending", "admin_seen", "accepted"].includes(r.status) && (
//                             <button disabled={actionId === r._id} onClick={() => setPdfModal({ id: r._id, url: "" })}
//                               className="flex items-center gap-1 rounded-lg bg-purple-500 px-2.5 py-1.5 text-[11px] font-bold text-white shadow-sm transition-all hover:bg-purple-600 disabled:opacity-60">
//                               <FileUp size={13} /> PDF পাঠান
//                             </button>
//                           )}
//                           {isCompleted && (
//                             <button disabled={actionId === r._id} onClick={() => handleAdminDelete(r._id, r.status)}
//                               className="flex items-center gap-1 rounded-lg bg-gray-500 px-2.5 py-1.5 text-[11px] font-bold text-white shadow-sm transition-all hover:bg-gray-600 disabled:opacity-60"
//                               title="Admin তালিকা থেকে সরান">
//                               <Trash2 size={13} /> সরান
//                             </button>
...
//           <div className="w-full max-w-md rounded-2xl border border-border bg-card p-6 shadow-xl">
//             <h2 className="mb-4 text-base font-bold text-foreground">বাতিলের কারণ লিখুন</h2>
//             <textarea rows={3} placeholder="কারণ লিখুন (কমপক্ষে ৩ অক্ষর)" value={cancelModal.note}
//               onChange={(e) => setCancelModal({ ...cancelModal, note: e.target.value })}
//               className="w-full rounded-xl border border-border bg-background p-3 text-sm text-foreground outline-none focus:ring-2 focus:ring-rose-500/30 placeholder:text-muted-foreground" />
//             <p className="mt-2 text-xs text-muted-foreground">Request বাতিল হলে ৳৫০ স্বয়ংক্রিয়ভাবে user-কে ফেরত দেওয়া হবে।</p>
//             <div className="mt-4 flex justify-end gap-2">
...
//           <div className="w-full max-w-md rounded-2xl border border-border bg-card p-6 shadow-xl">
//             <h2 className="mb-4 text-base font-bold text-foreground">PDF URL দিন</h2>
//             <input type="url" placeholder="https://... (PDF link)" value={pdfModal.url}
//               onChange={(e) => setPdfModal({ ...pdfModal, url: e.target.value })}
//               className="w-full rounded-xl border border-border bg-background p-3 text-sm text-foreground outline-none focus:ring-2 focus:ring-purple-500/30 placeholder:text-muted-foreground" />
//             <p className="mt-2 text-xs text-muted-foreground">Google Drive, Dropbox বা যেকোনো public PDF link দিন।</p>
//             <div className="mt-4 flex justify-end gap-2">
//               <button onClick={() => setPdfModal(null)} className="rounded-lg border border-border px-4 py-2 text-sm text-muted-foreground hover:bg-muted">বাতিল করুন</button>
//               <button onClick={handleSendPdf} className="rounded-lg bg-purple-500 px-4 py-2 text-sm font-bold text-white hover:bg-purple-600">PDF পাঠান</button>
//             </div>
//           </div>
//         </div>
//       )}
